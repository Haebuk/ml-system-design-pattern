...
@router.get("/projects/id/{project_name}")
def project_by_name(
    project_name: str,
    db: Session = Depends(get_db),
):
...
def experiment_by_model_version_id(
    model_version_id: str, db: Session = Depends(get_db)
):
    return cruds.select_experiment_by_model_version_id(
        db=db, model_version_id=model_version_id
    )


@router.get("/experiments/model-id/{model_id}")
def experiment_by_model_id(model_id: str, db: Session = Depends(get_db)):
    return cruds.select_experiment_by_model_id(db=db, experiment_model_id=model_id)
...
def update_evaluations(
    ...
    return cruds.update_experiment_evaluation(
        db=db, experiment_id=experiment_id, evaluations=evaluations.evaluations
    )

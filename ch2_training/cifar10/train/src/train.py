...
def start_run(
    ...
    logger.info(f"Latest Performance: Accuracy: {accuracy:.2f}%, Loss: {loss:.2f}")

    writer.close()

    model_file_name = os.path.join(
        downstream_directory, f"cifar10_{mlflow_experiment_id}.pth"
    )
    onnx_file_name = os.path.join(
        downstream_directory, f"cifar10_{mlflow_experiment_id}.onnx"
    )

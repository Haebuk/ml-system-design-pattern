...
class Cifar10Dataset(Dataset):
    ...
    def __load_image_files_and_labels(self):
        class_directories = [
            i for i in os.listdir(self.data_directory) if i.isdecimal()
        ]
        filepath_list = []
...
def train(
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            writer.add_scalar(
                "Loss/train", float(running_loss / (i + 1)), (epoch + 1) * i
            )

            if i % 200 == 199:
                end = time.time()
                logger.info(
                    f"[{epoch}, {i}] loss: {running_loss / 200} duration: {end - start}"
                )
                running_loss = 0.0
                start = time.time()
        epoch_end = time.time()
        logger.info(f"Epoch {epoch} finished in {epoch_end - epoch_start} seconds")
        ...
        checkpoints = os.path.join(
            checkpoints_directory, f"epoch_{epoch}_loss_{loss}.pth"
        )
        logger.info(f"Saving checkpoint: {checkpoints}")

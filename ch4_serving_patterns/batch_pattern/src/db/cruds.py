...
def select_without_prediction(db: Session) -> List[schemas.Item]:
    return db.query(models.Item).filter(models.Item.prediction is None).all()


def select_with_prediction(db: Session) -> List[schemas.Item]:
    return db.query(models.Item).filter(models.Item.prediction is not None).all()
...
def register_items(db: Session, items: List[schemas.ItemBase], commit: bool = True):
    for item in items:
        register_item(db, item, commit=commit)
...
def register_predictions(
    ...
    for id, prediction in predictions.items():
        _item = select_by_id(db=db, id=id)
        _item.prediction = prediction
        if commit:
            db.commit()
            db.refresh(_item)

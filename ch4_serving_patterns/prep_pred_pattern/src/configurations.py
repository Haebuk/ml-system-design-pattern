...
class APIConfigurations:
    ...
    description = os.getenv(
        "API_DESCRIPTION", "machine learning system serving patterns"
    )
...
class ModelConfigurations:
    ...
    label_path = os.getenv(
        "LABEL_PATH", "/prep_pred_pattern/data/image_net_labels.json"
    )

    sample_image_path = os.getenv(
        "SAMPLE_IMAGE_PATH", "/prep_pred_pattern/data/cat.jpg"
    )
    sample_image = Image.open(sample_image_path)

    preprocess_transformer_path = os.getenv(
        "PREPROCESS_TRANSFORMER_PATH",
        "/prep_pred_pattern/models/preprocess_transformer.pkl",
    )

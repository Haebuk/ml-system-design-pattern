...
def request_grpc(
    ...
    base64_image = base64.urlsafe_b64decode(image)
    request.inputs["image"].CopyFrom(tf.make_tensor_proto([base64_image]))
    response = stub.Predict(request, timeout=timeout_second)
...
def request_rest(
    ...
    response = requests.post(serving_address, json.dumps(request_dict), headers=headers)
